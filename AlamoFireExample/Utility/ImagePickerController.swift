...
protocol ImageCallBackDelegate {
    func getImageData(image: NSData)
}

public class ImagePickerController: UIView, UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    
    var imagedelegate:ImageCallBackDelegate?
...
//    }
    ...
    public func imagePickerController(_ picker: UIImagePickerController, didFinishPickingMediaWithInfo info: [UIImagePickerController.InfoKey : Any]) {
        if let image = info[UIImagePickerController.InfoKey.originalImage] as? UIImage{
            setImageForImageView(image: image)
            print("Image Select :: \(image)")
            let image1 = image.resizeWithWidth(width: 300)!
            guard let imageData = image1.jpegData(compressionQuality: 0.50) else { return }
            self.dataImage = imageData as NSData
            // Mark: Call back image data to parent controller
            imagedelegate?.getImageData(image: self.dataImage)
            UIApplication.topViewController()!.dismiss(animated: true, completion:  nil)
        }
        else{
            print("error")
